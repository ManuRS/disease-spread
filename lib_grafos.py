...
def fix_no_vecinos(grafo):
    for i in range (tam_grafo(grafo)):
        vecinos = adyacentes(grafo, i)
        if len(vecinos)==0:
            num = rd.randint(0, tam_grafo(grafo))
            if i==num:
                num = rd.randint(0, tam_grafo(grafo))
            grafo[i][num]=1
            grafo[num][i]=1
...
def scale_free (num_inicial, num_final, num_ramas):
    return libre_escala (num_inicial, num_final, num_ramas)
...
def rewired_1D (num_nodos, k, probabilidad):
    res = create_lattice_1D (num_nodos, k)
    k=k/2
    return rewired_aux(res, num_nodos, probabilidad)
    
def rewired_aux(res, num_nodos, probabilidad):     
    for i in range(num_nodos):
        vecinos = adyacentes(res, i)
        for vecino in vecinos:
            if vecino>i:
...
def small_word_1D(num_nodos, k, probabilidad):
    return rewired_1D (num_nodos, k, probabilidad)
            
def create_lattice_2D (num_nodos, k):
    k = int(k/4)
    res = np.zeros(shape = (num_nodos, num_nodos))
    largo = int(num_nodos**(0.5))
    
    for i in range(num_nodos):
        
        for ii in range(num_nodos):
                fin_linea = ii*largo + largo-1
                ini_linea = ii*largo
                if fin_linea>=i:
                    break
        
        for j in range(1, k+1):  
            if i%largo!=0 and i-j>=ini_linea: #El cero es que es el primero
                res[i][i-j]=1
            if i%largo!=largo-1 and i+j<=fin_linea: #largo-1 el ultimo 
                res[i][i+j]=1
                
        for j in range(1, k+1):
            if i-(j*largo) >= 0:
                res[i-(j*largo)][i]=1
            if i+(j*largo) < num_nodos:
                res[i+(j*largo)][i]=1
                
    return res
 
def rewired_2D (num_nodos, k, probabilidad):
    res = create_lattice_2D (num_nodos, k)
    k = int(k/4)
    return rewired_aux(res, num_nodos, probabilidad)

def small_word_2D(num_nodos, k, probabilidad):
    return rewired_2D (num_nodos, k, probabilidad)
